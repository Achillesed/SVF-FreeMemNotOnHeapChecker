...
bool ProgSlice::AllPathReachableSolve()
{
    ...
    while(!worklist.empty())
    {
        ...
        for(SVFGNode::const_iterator it = node->OutEdgeBegin(), eit = node->OutEdgeEnd(); it!=eit; ++it)
        {
            ...
            {
                Condition vfCond;
                const SVFBasicBlock* nodeBB = getSVFGNodeBB(node); //global，为空 
                const SVFBasicBlock* succBB = getSVFGNodeBB(succ);  //gep节点，这个gep为什么会生成，为for.end { basic block: for.end  }
                //outs()<<* nodeBB<<"\n";
                outs()<<* succBB<<"\n";
...
bool ProgSlice::isSatisfiableForPairs()
{
    ...
    for(SVFGNodeSetIter it = sinksBegin(), eit = sinksEnd(); it!=eit; ++it)
    {
        for(SVFGNodeSetIter sit = it, esit = sinksEnd(); sit!=esit; ++sit)
        {
            ...
            if(!isEquivalentBranchCond(guard, getFalseCond())) //表示guard不满足，即两个汇点的条件不能同时满足，不能同时到达两个汇点
            {
                setFinalCond(guard);    //将guard设置为最终条件
                return false;
            }
        }
    }

    return true; //满足，能同时到达两个汇点
}
