...
static Option<bool> FREEMEMNOTONHEAPCHECKER(
    "freememnotonheap",
    "Free Memory Not on Heap Detection",
    false
);
...
int main(int argc, char ** argv)
{
    ...
    if(FREEMEMNOTONHEAPCHECKER())
        saber=std::make_unique<FreeMemoryNotonHeapChecker>();
    else if(FILECHECKER())
        ...
        saber = std::make_unique<FreeMemoryNotonHeapChecker>();  // if no checker is specified, we use leak checker as the default one.
